 ...
 * @package Controller
 */
class RPC_Controller_Dispatcher
{
	...
	{
		return $this->response ? $this->response : RPC_HTTP_Response::getInstance();
	}
	...
	public function dispatch()
	{
		/*
			...
			the first letter upper cased and the Command string appended
		*/
		...
		*/
		$command = $cmdparts[$cmdkey] = ucfirst( $cmdparts[$cmdkey] ) . 'Command';
		...
		*/
		...
		{
			RPC_Signal::emit( array( 'RPC_Controller_Command', '404' ) );
		...
		*/
		...
		$command = new $command;
		if( ! $command instanceof RPC_Controller_Command )
		{
			throw new Exception( 'Class "' . $command . '" has to inherit from RPC_Controller_Command' );
		}
