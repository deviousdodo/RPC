 ...
 * @package Db
 */
abstract class RPC_Db_Table_Adapter implements Countable
{
	 ...
	 * @var string
	 */
	protected $rowclass = 'RPC_Db_Table_Row';
	 ...
	 * @return bool
	 */
	abstract public function unique( $column, RPC_Db_Table_Row $row );
	...
	public function __construct()
	{
		...
		{
			$name = str_replace( 'model', '', strtolower( get_class( $this ) ) );
		...
		$this->setIdentityMap( new RPC_Db_Table_RowMap() );
	}
	 ...
	 */
	protected function setDb( RPC_Db_Adapter $db )
	{
		$this->db = $db;
	}
	 ...
	 */
	public function setIdentityMap( RPC_Db_Table_RowMap $map )
	{
		$this->map = $map;
	}
	...
	public function create( $data = array() )
	{
		...
		{
			throw new BadArgumentException( 'If given, data must be an array' );
		}
	 ...
	 * @return bool
	 */
	public function validate( RPC_Db_Table_Row $row )
	{
		$pk = $row->getPk();
	...
	public function update( $row )
	{
		...
		if( ! $this->onBeforeSave( $row, RPC_Db::QUERY_UPDATE ) )
		{
			$this->getDb()->rollback();
			return false;
		}
		...
		if( ! $this->onAfterSave( $row, RPC_Db::QUERY_UPDATE ) )
		{
			$this->getDb()->rollback();
			return false;
		}
