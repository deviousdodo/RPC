...
abstract class RPC_Db_Adapter
{
	 ...
	 * @var int
	 */
	protected $_rpc_fetchmode = RPC_Db::FETCH_ASSOC;
	...
	{
		if( ! RPC_Signal::emit( array( 'RPC_Db', 'query_start' ), array( $sql, 'statement' ) ) )
		{
			return 0;
		}
		...
		RPC_Signal::emit( array( 'RPC_Db', 'query_end' ), array( $sql, 'statement' ) );
	...
	{
		if( ! RPC_Signal::emit( array( 'RPC_Db', 'query_start' ), array( $sql, 'query' ) ) )
		{
			return null;
		}
		...
		RPC_Signal::emit( array( 'RPC_Db', 'query_end' ), array( $sql, 'query' ) );
